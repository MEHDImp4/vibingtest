...
- `src/native`: Python helper for global hotkeys, microphone recording, active-window detection, and paste injection.

## Runtime Flow

1. The Electron main process starts and creates the tray plus hidden overlay window.
2. `NativeBridge` launches `src/native/helper.py` and sends configured hotkeys through environment variables or IPC commands.
...
- The native helper stays intentionally small and JSON-line based, so it can later be replaced by a Rust binary without changing renderer code.
- Provider-specific logic is isolated in `src/main/ai-pipeline.ts`.
- LLM rewriting can be disabled by selecting `None`, which pastes the raw ASR transcript. Translation mode also falls back to raw transcript when LLM is disabled.
